#!/usr/bin/env python
#########################################################################
# File Name: circSHAPEtrim.py
# Author: FangNan
# mail: nanfang@picb.ac.cn
#########################################################################
import argparse
import re
from string import maketrans

parser = argparse.ArgumentParser("Trim primers from alignment file")
parser.add_argument("-i","--input",type=str, help="sam file")
parser.add_argument("-p","--primer",type=str, help="primer file, each line is a primer sequence(ATCG without "U")" )
parser.add_argument("-o","--out",type=str, help="output file name, out.sam")
...
def subPrimer(Line,rePattern):        
        ...
        MDstr=re.findall(r'[0-9]+|[A|T|C|G|^]+',MDlist[-1])
        newMDstr=MDstr[:]
